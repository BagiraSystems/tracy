...
The \texttt{-x, -\hspace{-1.25ex} -export <file>} option switches to a separate mode intended for large traces. Instead of printing to stdout, it writes one row per zone occurrence to \texttt{<file>}, and every string (zone name, source file, thread or GPU context name, zone text) is replaced by an index into a dictionary written to \texttt{<file>.dict}. The dictionary is a two-column CSV (\texttt{index}, \texttt{string}) using the same separator; its strings are quoted so paths or texts containing the separator survive. The row columns are:

\begin{itemize}
  \item \texttt{name}, \texttt{src\_file}, \texttt{thread}, \texttt{value} -- Dictionary indices (\texttt{value} is empty when the zone has no text)
  \item \texttt{src\_line}, \texttt{ns\_since\_start}, \texttt{exec\_time\_ns} -- Plain numbers
  \item \texttt{gpu} -- 1 for GPU zones, 0 for CPU zones; the \texttt{thread} of a GPU zone is its GPU context name
\end{itemize}
