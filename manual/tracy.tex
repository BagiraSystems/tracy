...
\begin{itemize}
  ...
  \item \texttt{-B, -\hspace{-1.25ex} -begin-frame <frame>} -- Start the window at the begin of the given frame of the main frame set, using the profiler's frame numbering (the same numbers the \texttt{frame} column prints); defaults to the first frame of the trace when only \texttt{-n} or \texttt{-E} is given
