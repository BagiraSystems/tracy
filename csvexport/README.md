...
(`tracy-csvexport 0.14.1 / <hash> / bagira.2`). Releases are tagged
...
Since `bagira.2` the build excludes the capstone disassembler (only needed for live capture,
which this tool never does), shrinking the executable from 14 MB to under 1 MB.
